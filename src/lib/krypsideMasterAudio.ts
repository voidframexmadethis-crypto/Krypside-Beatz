/**
 * 🔊 KRYPSIDE MASTER AUDIO HYBRID ENGINE
...
class KrypsideMasterAudioEngine {
  private static instance: KrypsideMasterAudioEngine;
  ...
  public static getInstance(): KrypsideMasterAudioEngine {
    if (!KrypsideMasterAudioEngine.instance) {
      KrypsideMasterAudioEngine.instance = new KrypsideMasterAudioEngine();
    }
    return KrypsideMasterAudioEngine.instance;
  }
  ...
  public initContext(): AudioContext | null {
    ...
    try {
      ...
      console.warn("[KrypsideMasterAudio] Web Audio DSP initialization fallback notice:", e);
    }
  ...
  private setupAudioListeners() {
    ...
    this.audioElement.addEventListener('loadedmetadata', () => this.notifyListeners('loadedmetadata', { duration: this.audioElement.duration }));
    ...
    this.audioElement.addEventListener('error', (e) => this.notifyListeners('error', { error: this.audioElement.error }));
  ...
  public async play(target?: string | any): Promise<void> {
    ...
    if (typeof target === 'string') {
      ...
      if (window.resolveKrypsideAudioStream) {
        url = window.resolveKrypsideAudioStream(target);
      } else {
        url = target.watermarkedAudioUrl || target.audioUrl || target.src || '';
      }
    }
    ...
    if (this.audioElement.src) {
      return this.audioElement.play().catch(err => {
        console.warn("[KrypsideMasterAudio] Playback interrupted or requires user gesture:", err);
        throw err;
      });
    }
  }
  ...
  public async preload(url: string): Promise<AudioBuffer | null> {
    ...
    try {
      ...
      console.warn("[KrypsideMasterAudio] Preload decode notice:", e);
      return null;
    }
  }
  ...
  /**
   * Backward Compatibility Bridge Method for window.KrypsidePlayer
   */
...
export const krypsideMasterAudio = KrypsideMasterAudioEngine.getInstance();

// Attach to Global Scope if running in browser
if (typeof window !== 'undefined') {
  (window as any).KrypsideMasterAudio = krypsideMasterAudio;
  
  // Maintain backward compatibility for window.KrypsidePlayer
  (window as any).KrypsidePlayer = {
    playBeat: (url: string) => krypsideMasterAudio.playBeat(url),
    pause: () => krypsideMasterAudio.pause(),
    setVolume: (val: number) => krypsideMasterAudio.setVolume(val),
    seek: (time: number) => krypsideMasterAudio.seek(time),
    play: (target?: any) => krypsideMasterAudio.play(target),
    getStats: () => krypsideMasterAudio.getStats()
  };
}

export default krypsideMasterAudio;
